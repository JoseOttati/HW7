 ...
 ********************************************************************/
 ...
 public class ProblemSolutions {
     ...
     private void mergeDivisbleByKFirst(int arr[], int k, int left, int mid, int right)
     {
      ...
      // First merge divisible elements from both halves in their original order
      while (i < leftArray.length && j < rightArray.length) {
          boolean leftDiv = (leftArray[i] % k == 0);
          boolean rightDiv = (rightArray[j] % k == 0);
          
          if (leftDiv && rightDiv) {
              // Both divisible - maintain original order
              arr[pos++] = leftArray[i++];
          } else if (leftDiv) {
              arr[pos++] = leftArray[i++];
          } else if (rightDiv) {
              arr[pos++] = rightArray[j++];
          } else {
              break;
          }
      }

      // Finish remaining divisible elements in left array
      while (i < leftArray.length && leftArray[i] % k == 0) {
          arr[pos++] = leftArray[i++];
      }

      // Finish remaining divisible elements in right array
      while (j < rightArray.length && rightArray[j] % k == 0) {
          arr[pos++] = rightArray[j++];
      }

      // Now sort non-divisible elements in ascending order
      Arrays.sort(leftArray, i, leftArray.length);
      Arrays.sort(rightArray, j, rightArray.length);

      // Merge remaining non-divisible elements
      while (i < leftArray.length && j < rightArray.length) {
